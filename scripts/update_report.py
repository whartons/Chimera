...
COMFY_REF = "0.33.3"   # the reference build documented in docs/STACK.md / SETUP.md
...
def build(rows, repo="whartons/Chimera"):
    ...
    lines = [
        ...
        *[f"- {MARK.get(lvl, 'ℹ️')} {msg}" for lvl, msg in rows],
        ...
        "- **Optional ComfyUI judge (ComfyUI-QwenVL)** — no longer the default (the agent judge runs on "
        "Ollama Qwen3-VL). If you use the ComfyUI judge node, re-pin it to a Qwen3-VL-capable commit "
        "(≥ v1.0.4) + re-run the node-pack scan before bumping — it is NOT auto-checked here.",
        "",
