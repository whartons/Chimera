...
|------|------|--------------|---------|---------------|
| **ComfyUI-LTXVideo** | `Lightricks/ComfyUI-LTXVideo` | `3b9c5cd` | video (LTX-2.3 i2v + synced audio + latent upscaler) | safe-with-precautions — never use the cloud `GemmaAPITextEncode`; avoid the prompt-enhancer's `trust_remote_code`. Re-audited `4f45fd6 → 3b9c5cd` (4 commits): docs/example-workflow additions + one behavior-preserving RoPE import-compat shim (protective for future ComfyUI core rope refactor); no node/API changes |
...
|--------|---------|---------|-----------|------------------|
| **comfyui-mcp** | npm `comfyui-mcp` (`artokun`) | **0.18.0** | stdio, **loopback only** (127.0.0.1) | MIT; audited not-malicious. Real controls: per-tool **approval gates** on code-exec tools (`.claude/settings.json`) + loopback-only binding + env-gating of the optional cloud/tunnel/Claude-Agent-SDK features (they stay inert unless explicitly enabled); benign `postinstall` (copies a settings template). `.mcp.json` sets **`NPM_CONFIG_INCLUDE=optional`** (replacing `NPM_CONFIG_OMIT=optional`, which on node ≥ 24 stripped `sharp`'s required native binary `@img/sharp-*` and crashed the server); the optional cloud/tunnel/agent deps install but stay inert. Pinned + re-audited on bump; **0.20.x–0.48.x held** (issues #38/#41/#44) — 0.20.9 has a broken upstream import (`@stable-canvas/comfyui-client/dist/main.modern.mjs`, absent from client `1.5.9`); 0.24.5+ adds **default-on startup self-update** (phones `registry.npmjs.org`, can `npm i @latest` on global/local installs) and **panel auto-install** (mutates the ComfyUI install from an unpinned "nightly" channel), and 0.48.x layers a large multi-provider agent/network surface on top. `.mcp.json` sets the opt-outs `COMFYUI_MCP_AUTOUPDATE=0` + `COMFYUI_MCP_PANEL_AUTOINSTALL=0` as defense-in-depth (inert on 0.18.0; they neutralize both behaviors if the pin ever drifts). See `modules/agent/README.md`. |
...
| **freecad-mcp** | GitHub `neka-nat/freecad-mcp` (server + addon) | **`4c3f2ef`** (v0.1.19) | stdio → XML-RPC, **loopback** (127.0.0.1:9875) | MIT; no telemetry (deps `mcp[cli]`/`validators`), from-source audited. Tier-1/2 tools gated; keep "Remote Connections" OFF. Pinned by commit; re-audited on bump (last: `1697aff → 4c3f2ef`, 10 commits — error-handling/GUI-robustness only, no new tools/deps; re-copy the addon half into FreeCAD `Mod/`). |
