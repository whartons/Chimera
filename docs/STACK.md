...
|------|------|--------------|---------|---------------|
| **ComfyUI-LTXVideo** | `Lightricks/ComfyUI-LTXVideo` | `15d09ab` | video (LTX-2.3 i2v + synced audio + latent upscaler) | safe-with-precautions — never use the cloud `GemmaAPITextEncode`; avoid the prompt-enhancer's `trust_remote_code`. Re-audited `3b9c5cd → 15d09ab` (4 commits): LTX-2.5 example workflows (inert JSON) + legacy RoPE helpers made self-contained (`rope_utils.py`, pure math) + Gemma-loader dir-resolution/left-padding fixes; no new nodes, deps, or network |
...
|--------|---------|---------|-----------|------------------|
| **comfyui-mcp** | npm `comfyui-mcp` (`artokun`) | **0.18.0** | stdio, **loopback only** (127.0.0.1) | MIT; audited not-malicious. Real controls: per-tool **approval gates** on code-exec tools (`.claude/settings.json`) + loopback-only binding + env-gating of the optional cloud/tunnel/Claude-Agent-SDK features (they stay inert unless explicitly enabled); benign `postinstall` (copies a settings template). `.mcp.json` sets **`NPM_CONFIG_INCLUDE=optional`** (replacing `NPM_CONFIG_OMIT=optional`, which on node ≥ 24 stripped `sharp`'s required native binary `@img/sharp-*` and crashed the server); the optional cloud/tunnel/agent deps install but stay inert. Pinned + re-audited on bump; **0.20.x–0.50.x held** (issues #38/#41/#44/#49) — 0.20.9 has a broken upstream import (`@stable-canvas/comfyui-client/dist/main.modern.mjs`, absent from client `1.5.9`); 0.24.5+ adds **default-on startup self-update** (phones `registry.npmjs.org`, can `npm i @latest` on global/local installs) and **panel auto-install** (mutates the ComfyUI install from an unpinned "nightly" channel), 0.48.x layers a large multi-provider agent/network surface on top, and 0.50.x makes `npx` installs auto-update-ELIGIBLE (our exact launch mode). `.mcp.json` sets the opt-outs `COMFYUI_MCP_AUTOUPDATE=0` + `COMFYUI_MCP_PANEL_AUTOINSTALL=0` as defense-in-depth (inert on 0.18.0; they neutralize both behaviors if the pin ever drifts). See `modules/agent/README.md`. |
| **blender_mcp** | Gitea `lab/blender_mcp` (Blender Foundation) | **v1.0.0 +5** (`4309a396`) | stdio → TCP, **loopback** (127.0.0.1:9876) | GPL-3.0; first-party, **zero telemetry** (deps `docutils`/`mcp[cli]`/`pyyaml`), from-source audited. Tier-1/2 code-exec & path tools gated; launched from the pinned Gitea commit via `uvx --with "mcp[cli]<2" --from "git+…#subdirectory=mcp"` (the `--with` pins the MCP SDK to 1.x — upstream imports the SDK-1.x `mcp.server.fastmcp` path and does not bound the dep) (server pkg is in the repo's `mcp/` subdir; **never** the bare `uvx blender-mcp`); never `-t http`. **Gitea, not GitHub.** |
| **freecad-mcp** | GitHub `neka-nat/freecad-mcp` (server + addon) | **`3745ff9`** (v0.1.22) | stdio → XML-RPC, **loopback** (127.0.0.1:9875) | MIT; no telemetry (deps `mcp[cli]`/`validators`), from-source audited. Tier-1/2 tools gated; keep "Remote Connections" OFF. Pinned by commit; re-audited on bump (last: `4c3f2ef → 3745ff9`, 27 commits — v0.1.22: stuck-GUI-task fail-fast + a NEW read-only `get_rpc_status` health tool (Tier 3, ungated), screenshot token-saving params + 1024px auto-cap, Wayland framebuffer capture, actual-name reporting, socket stop/close fixes; no new runtime deps, no new exec/network; re-copy the addon half into FreeCAD `Mod/`). |
