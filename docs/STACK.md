...
flowchart TD
    ...
    mcp["comfyui-mcp 0.20.9 (stdio, loopback)"]
...
|--------|---------|---------|-----------|------------------|
| **comfyui-mcp** | npm `comfyui-mcp` (`artokun`) | **0.20.9** | stdio, **loopback only** (127.0.0.1) | MIT; audited not-malicious. Per-tool **approval gates** on code-exec tools (`.claude/settings.json`); `NPM_CONFIG_OMIT=optional` omits the optional cloud/tunnel/Claude-Agent-SDK/codex deps (their cloud/Civitai/auth/orchestrator features stay inert); benign `postinstall` (copies a settings template). **`COMFYUI_MCP_AUTOUPDATE=0`** neutralises 0.20.9's startup self-update (which would otherwise auto-pull `@latest` and defeat pinning — inert anyway under our `npx` launch). Pinned + re-audited on bump. |
