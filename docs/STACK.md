...
|--------|---------|---------|-----------|------------------|
| **comfyui-mcp** | npm `comfyui-mcp` (`artokun`) | **0.18.0** | stdio, **loopback only** (127.0.0.1) | MIT; audited not-malicious. Real controls: per-tool **approval gates** on code-exec tools (`.claude/settings.json`) + loopback-only binding + env-gating of the optional cloud/tunnel/Claude-Agent-SDK features (they stay inert unless explicitly enabled); benign `postinstall` (copies a settings template). `NPM_CONFIG_OMIT=optional` was **removed** — on node ≥ 24 it strips `sharp`'s required native binary (`@img/sharp-*`, shipped as an optionalDependency) and crashes the server on startup. Pinned + re-audited on bump; **0.20.x–0.24.x held** — 0.20.9 has a broken upstream import (`@stable-canvas/comfyui-client/dist/main.modern.mjs`, absent from client `1.5.9`) and offers nothing this repo uses (see `modules/agent/README.md`). |
...
| **freecad-mcp** | GitHub `neka-nat/freecad-mcp` (server + addon) | **`1697aff`** (v0.1.18 + 4 bug-fix commits) | stdio → XML-RPC, **loopback** (127.0.0.1:9875) | MIT; no telemetry (deps `mcp[cli]`/`validators`), from-source audited. Tier-1/2 tools gated; keep "Remote Connections" OFF. Pinned by commit; re-audited on bump. |
